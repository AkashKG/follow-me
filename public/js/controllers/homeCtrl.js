angular.module('homeCtrl', [])

.controller('homeController', function ($scope, $window) {
    
    $scope.$parent.seo = { 
    	    title : 'fodo | take your task', 
    	    descripton: 'fodo, take your task wherever you go!' 
    	  }; 
    $scope.certifications=[
    	{
        	title:"M101x",
        	link:"https://courses.edx.org/certificates/999610212bf14b30bf79b3dfabafe2c1",
        	description:"Introduction to MongoDB using mean Stack"
        },
        {
        	title:"KIIT Portal",
        	link:"http://www.getjar.com/categories/education-apps/more/KIIT-Portal-875949",
        	description:"Android app for tracking GPA's and few other features"
        },{
        	title:"Angular Calculator",
        	link:"http://s.codepen.io/AkashKG/debug/ZQELoB",
        	description:"Angular Calculator focussing more on UI."
        }
    ]
    $scope.timeline=[{
    	title:"fodo.herokuapp.com",
    	link:'https://fodo.herokuapp.com',
    	description:"Individual project. A self tracking todo app. Here your can create your  notebooks. You can add To-Do's to notebook and tasks to To-Do's. Once you complete it, check it."
    },{
    	title:"Book-barter",
    	link:'https://github.com/AkashKG/bookbars',
    	description:"Team Lead. Full stack developer. A platform where users can share their books in the near locality."
    },{
    	title:"Edulimn | Private repository",
    	link:"_",
    	description:"Front End developer. MOOC as a SaaS based Concept."
    		
    },{
    	title:"SRF-2015 Indian Academy",
    	description:"Real time task scheduling on heterogeneous multiprocessors.",
    	link:"http://web-japps.ias.ac.in:8080/fellowship2015/lists/selectedList.jsp"
    }]
    $scope.show={
    		timeline:true,
    		existence:true,
    		cert:true,
    		existence_icon:'visibility_off',
    		timeline_icon:"visibility_off",
    		cert_icon:"visibility_off"
    		//existence-icon-color:''
    }
    $scope.techSwitch = true;
    $scope.toggleExistence=function(){
    	$scope.show.existence=!$scope.show.existence;
    	if($scope.show.existence_icon=='visibility_off'){
    		$scope.show.existence_icon='visibility_on';
    	}
    	else{
    		$scope.show.existence_icon='visibility_off';
    	}
    }
    $scope.toggleCert=function(){
    	$scope.show.cert=!$scope.show.cert;
    	if($scope.show.cert_icon=='visibility_off'){
    		$scope.show.cert_icon='visibility_on';
    	}
    	else{
    		$scope.show.cert_icon='visibility_off';
    	}
    }
    $scope.toggleTimeline=function(){
    	$scope.show.timeline=!$scope.show.timeline;
    	if($scope.show.timeline_icon=='visibility_off'){
    		$scope.show.timeline_icon='visibility_on';
    	}
    	else{
    		$scope.show.timeline_icon='visibility_off';
    	}
    }
    $scope.existence=[{
    	title:'github.com/AkashKG',
    	link:'https://github.com/AkashKG',
    	icon:'link'
    },{
    	title:'linkedIn',
    	link:'https://linkedin.com/yesitsakash',
    	icon:'link'
    },{
    	title:'codepen.io/AkashKG',
    	link:'https://codepen.io/AkashKG',
    	icon:'link'
    },{
    	title:'quora',
    	link:'https://quora.com/akash-gupta-10',
    	icon:'link'
    }]
    $scope.openLink=function(url){
    	$scope.link=url;
    	$window.open(url,'_blank');
    }
    
    $scope.thingsILove=[{
    	name:'AngularJS',
    	icon:'http://ccoenraets.github.io/pgday14/images/angular-icon.png',
    	description:'I have worked on one major project and 3 minor project using it.'
    },{
    	name:'MongoDB',
    	icon:'http://blogs.msdn.com/cfs-filesystemfile.ashx/__key/communityserver-blogs-components-weblogfiles/00-00-00-60-29-metablogapi/4846.mongodb_2D00_logo_5F00_thumb_5F00_3BA2A1E5.png',
    	description:'Certification Course - M101x: Introduction to mongoDB using MEAN stack.'
    },{
    	name:'NodeJS',
    	icon:'https://cdn2.iconfinder.com/data/icons/nodejs-1/128/nodejs-128.png',
    	description:'Used Express npm package while working with NodeJS in web development.'
    },{
    	name:'Angular Material',
    	icon:'https://s3-us-west-2.amazonaws.com/s.cdpn.io/t-114/profile/profile-512_2.jpg',
    	description:'Made responsive webapps using it. Press F12 and then ctrl+shift+m to see the responsiveness of this page.'
    }]
    $scope.programming=[{
    	name:'C++',
    	icon:'data:image/png;base64,',
    	description:'First programming language that I learnt. (Proficient) '
    },{
    	name:'Java',
    	icon:"data:image/png;base64,",
    	description:'Learnt during the 4th semester of B.Tech course'
    },{
    	name:'javascript',
    	icon:'data:image/png;base64,',
    	description:'Learnt basic Javascript from freecodecamp'
    },{
    	name:'bootstrap',
    	icon:'https://bootstrapcreative.com/wp-bc/wp-content/uploads/2016/01/bootstrap-logo.png',
    	description:'End user UI at Edulimn'
    }]
    $scope.educationSwitch=true;
  });

